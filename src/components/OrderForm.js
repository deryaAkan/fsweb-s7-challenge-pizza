...
export default function NewPizzaForm({ handlePizzaOrder }) {
  ...
  const calculateTotalPrice = () => {
    const toppingsPrice = formData.toppings.reduce((total, topping) => {
      const matchingTopping = toppingsData.find(
        (t) => t.name.toLowerCase() === topping
      );
        ...
        <form onSubmit={handleSubmit} id="pizza-form">
            ...
            <div className="dough" id="dough-size">
              <div id="size-radio">
                ...
                <div className="radioButton-container">
                  {["S", "M", "L"].map((size) => (
                    <div key={size}>
                      <label className="containerRB" id={size === "S" ? "checkmarkRB1" : size === "M" ? "checkmarkRB2" : "checkmarkRB3"}>
                      <input
                        type="radio"
                        name="size"
                        value={size}
                        ...
                        required
                      /> <span className="checkmarkRB"></span>
                      </label>
                    </div>
                  ))}
                </div>
            ...
            <label>
              ...
              <div className="extra-toppings extras">
                {toppingsData.map((topping, index) => (
                  <div
                    className={`extra-toppings extras ${
                      index === 0 ? "invisible" : ""
                    }`}
                    key={index}
                  >
          ...
          <div className="counter-submit">
            ...
            <div className="submit-container">
              <div id="siparisToplami">
                <p>Sipariş Toplamı</p>
                <p>Seçimler {(formData.toppings.length * 5).toFixed(2)}₺</p>
                <p>
                  Toplam {" "}
                  {(
                    calculateTotalPrice() +
